/**
 * Suite seed for inscription tests.
 *
 * Creates all competitions needed by the inscription test suites:
 * - Happy Path: 1 individual category, registration closed
 * - UserIntent: 1 individual category, registration open
...
async function main() {
    ...
    const happyPath = await createCompetition(ctx, {
        ...baseCompetition('Happy Path Competition', 'Competition for happy path inscription E2E tests', false),
        categories: [individualCategory('500 pcs')],
    });

    // 2. UserIntent Individual
    const userIntent = await createCompetition(ctx, {
        ...baseCompetition('UserIntent Individual Competition', 'Tests registering a non-platform participant'),
    ...
    // 5. Refuse
    const refuseInscription = await createCompetition(ctx, {
        ...baseCompetition('Refuse Test Competition', 'Tests organizer refusing an inscription'),
    ...
    writeSeedOutput(import.meta.url, {
        ...
        userIntent: { competitionId: userIntent.id },
        groupTeam: { competitionId: groupTeam.id },
        unregister: { competitionId: unregister.id },
        refuseInscription: { competitionId: refuseInscription.id },
