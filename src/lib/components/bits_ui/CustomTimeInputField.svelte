...
<TimeField.Root bind:value bind:placeholder {...restProps}>
  <div class="flex w-full min-w-[280px] flex-col gap-1.5">
    ...
    <TimeField.Input
      {name}
      class="h-input rounded bg-primary-50-950 text-foreground focus-within:border-border-input-hover focus-within:shadow-date-field-focus hover:border-border-input-hover data-invalid:border-destructive flex w-full select-none items-center px-2 p-1 tracking-[0.01em] "
    >
