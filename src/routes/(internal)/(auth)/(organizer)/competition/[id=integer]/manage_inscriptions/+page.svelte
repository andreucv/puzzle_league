<script lang="ts">
    ...
    async function handleRemind(recordId: string, note?: string) {
        ...
        const minLoadingTime = new Promise(resolve => setTimeout(resolve, 500));
        try {
            const response = await fetch(`/api/inscriptions/${recordId}/remind`, {
                method: 'POST',
            ...
            if (response.ok && result.success) {
                showResultMessage({ success: true, message: $t('manage_inscriptions.remind_success', { count: result.remindedCount }) });
                await invalidateAll();
            } else {
                showResultMessage({ success: false, message: result.error || $t('manage_inscriptions.remind_error') });
            }
        } catch {
            showResultMessage({ success: false, message: $t('manage_inscriptions.remind_error') });
        } finally {
            processingRecordId = null;
        }
    }
    ...
    async function handleBulkRemind(categoryId: number, note?: string) {
        ...
        try {
            ...
            if (response.ok && result.success) {
                showResultMessage({ success: true, message: $t('manage_inscriptions.remind_bulk_success', { reminded: result.remindedCount, skipped: result.skippedCount }) });
                await invalidateAll();
            } else {
                showResultMessage({ success: false, message: result.error || $t('manage_inscriptions.remind_error') });
            }
