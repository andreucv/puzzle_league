<script lang="ts">
    ...
    let {
        ...
        onSuccess: (claimedIds: string[]) => void | Promise<void>;
    } = $props();
    ...
    async function handleClaimSuccess() {
        const claimedIds = Array.from(selectedIds);
        selectedIds = new Set();
        await onSuccess(claimedIds);
    }

    async function continueWithoutClaims() {
        selectedIds = new Set();
        await onSuccess([]);
    }
</script>

<div data-testid="onboarding-step-claim" class="space-y-4">
    ...
    {#if unclaimedIntents.length === 0}
        <Card>
            <div class="text-center py-6 space-y-4">
                <Icon icon="mdi:check-all" width="3rem" height="3rem" class="text-success-500 mx-auto" />
                <p class="text-surface-600 dark:text-surface-400">{$t('claim_participations.no_matches')}</p>
                <button
                    type="button"
                    class="btn preset-filled-primary-500 w-full"
                    onclick={continueWithoutClaims}
                    disabled={isSubmitting}
                    data-testid="onboarding-claim-continue"
                >
                    {$t('claim_participations.continue')}
                </button>
            </div>
        </Card>
    {:else}
        ...
        <div class="flex gap-3 pt-2">
            ...
            <form
                ...
                use:enhance={createEnhanceHandler(
                    ...
                    'error',
                    {
                        onSuccess: continueWithoutClaims,
                        setSubmitting: (v) => (isSubmitting = v),
                    }
                )}
            >
