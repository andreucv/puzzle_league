...
describe("Login Page", () => {
    ...
    it("shows password confirmation when switching to register mode", async () => {
        ...
        expect(
            screen.getByTestId("input-password-confirm"),
        ).toBeInTheDocument();
    ...
    it("shows error message on failed login", async () => {
        ...
        // The mock auth_client returns error.code 'INVALID_EMAIL_OR_PASSWORD', which the page
        // maps through getAuthErrorKey() to the i18n key 'auth.errors.invalid_credentials'.
        // The translations mock is an identity function, so the key is the rendered text.
        await vi.waitFor(
            () => {
    ...
    it("can switch back to login mode from register mode", async () => {
        ...
        expect(screen.getByTestId("login-submit")).toBeInTheDocument();
        expect(
            screen.queryByTestId("input-password-confirm"),
